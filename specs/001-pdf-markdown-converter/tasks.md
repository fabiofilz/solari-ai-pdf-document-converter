...
**2026-09-09 post-semantic remediation** (independent adversarial audit of checkpoint
`70aa0cf`; narrow remediation pass — **no T075+ work**). Confirmed findings closed with
the smallest conservative changes + adversarial regression tests, no task renumbered,
none removed, every T075–T161 stays `[ ]`:
- **R1 (BLOCKER)** `transform/tables.py` `_Builder._assign_cells` — two distinct source
  literals resolving to one grid anchor no longer retains only the first while marking
  both ids retained; the table fragment is abandoned conservatively (region flows to
  ordinary semantic processing, nothing consumed). Casefold/whitespace-equivalent
  evidence still collapses. Tests: `tests/unit/test_tables_r1_collision.py`.
- **R2 (HIGH)** `transform/artifacts.py` — repeated header/footer removal now also
  requires a contiguous occurrence run (discontinuous page selections such as pages 2
  & 9 ⇒ kept-ambiguous) and yields to a carried heading/subheading/caption hint. Frozen
  2-page contiguous margin-consistent furniture removal is unchanged.
- **R3 (HIGH)** `transform/artifacts.py` — auth-phrase / hex-hash / OCR-barcode SHAPE
  alone no longer deletes content; a page-margin-band signal is required, mid-body
  matches are kept-ambiguous.
- **R4 (MEDIUM)** `transform/structure.py` `infer_structure` — conflicting explicit
  heading levels are resolved only by the source's own numbering depth, never by
  extractor lexical order; duplicate `None`-level hints record one applied decision.
- **R5 (MEDIUM)** `transform/lists.py` — bare numeric/Roman clause prefixes require a
  clause-like body (`1.5 million residents…`, `I. am available…` ⇒ ordinary body).
- **R6 (MEDIUM)** `transform/tables.py` — `TableBlock.hint_decisions` sorted with the
  same canonical key as `TablesResult.hint_decisions`.
- **R7** `model/semantic.py` — `SemanticDocument.collapsed_headers` carries T070's
  `CollapsedHeader` records verbatim (frozen `RenderMap.collapsed_into` need; OCR
  indicator + per-segment render boundary lineage were **not** required — already
  available on the CED `SourceRef` / the ordered `provenance` + `segment_transforms`).
- **R8** `transform/build_semantic.py` — `literal_accounting_violations` +
  `LiteralAccountingError`: a literal-level fidelity invariant at the semantic boundary
  (raised by `build_semantic`) that detects a provenance-retained segment whose literal
...
- [x] T069 [P] [US1] Create `src/solari_converter/transform/lists.py` — list + nesting reconstruction; numbered article/clause identifier retention (FR-018/FR-019); records any hint use (depends on T022; tests T066 `test_lists.py`) — **DONE 2026-09-09 (S1)**: `reconstruct_lists(ced, reflow_result) -> ListResult` (`ListItemNode` / `ClauseNode` / `BodyNode`). Bullet / decimal / alpha markers + dotted-decimal / roman / keyword (`Article N`, `Cláusula 4ª`) clause identifiers detected from source text; identifiers kept **verbatim** — never renumbered, normalised, or reformatted (FR-019). Nesting from `bbox.x0` with `LIST_INDENT_STEP_MIN`; the flatter reading wins on a tie. A `list_item` hint is applied only when a real leading marker is present; recorded either way.
- [x] T070 [P] [US1] Create `src/solari_converter/transform/tables.py` — per-page table build; multi-page stitch; repeated-header collapse; merged-cell detect; preserve numerics (FR-020–FR-022); any re-order recorded as a `structural_reorder` entry carrying `scope` / `affected_segment_ids` / `from_order` / `to_order` / `reason`, distinct from the source order (SC-023) — this is the record the reading-order verification invariant (T144/T145 + M8) checks against (depends on T022; tests T066 `test_tables.py`) — **DONE 2026-09-09 (Semantic Block S2)**: `build_tables(ced, reflow_result) -> TablesResult`. CED accepted segments only — never split; never reopen the PDF; no LLM / no dependency / no Config / no run-identity fold. Detection is hint-gated (`table` / `table_row` / `table_cell`) plus geometry-included hint-less cells inside a hinted fragment bbox within `CELL_BBOX_TOLERANCE`; **no hint-free grid inference**. Deterministic top-anchored row bands + narrowest-first column clusters (named constants only); grid corroboration (≥2×2 + ≥3 filled) or the region stays prose with a `HintDecision(applied=False)` + `single_column_region_skipped`. Geometry-only `rowspan`/`colspan`; ambiguous/non-contiguous span → 1 + `span_uncertain`. Conservative `can_stitch` (adjacent pages, equal column count, compatible signatures within `COLUMN_X0_TOLERANCE`, no disallowed intervening content, positive continuation signal); page-number / repeated furniture is stitch-transparent (listed in the `StitchRecord`, **not** consumed, **not** removed). Repeated continuation header collapsed **once** (`FR-022`) with `CollapsedHeader` lineage; identical data rows and OCR-variant headers are not collapsed. Table-scoped `StructuralReorder` (`permitted_by` `FR-020` / `FR-021`) only when the accepted order differs from row-major. Deterministic `tf-` / `tbl-` sha1 ids; `TablesResult.consumed_segment_ids` for T073. **STOP GATE after T070** — T071/T072/T073 untouched.
- [x] T071 [P] [US1] Create `src/solari_converter/transform/artifacts.py` — detect + remove repeated headers/footers, page numbers, watermarks/backgrounds, auth stamps/barcodes/QR/vertical auth text; keep + record when ambiguous (FR-006–FR-011); emit `RemovalLog` entries (depends on T022; tests T066 `test_artifacts.py`) — **DONE 2026-09-09 (T072→T071→T073 block, dependency-safe order)**: `remove_artifacts(ced, reflow_result) -> ArtifactResult`. Consumes CED + the **non-table** `ReflowResult` (`reflow(ced, excluded_segment_ids=tables.consumed_segment_ids)`) — table-owned content is structurally absent, never reconsidered. Own, independent, stricter-than-T070-stitch-transparency evidence: repeated running header/footer requires exact comparison-key match, ≥`HEADER_FOOTER_MIN_PAGES`(2) pages, a **majority** of the document's pages, position-consistency (`FURNITURE_TOP_TOLERANCE`), and a consistent **margin band** (page-top or page-bottom extreme) — stitch transparency checks none of the margin/majority conditions. Page-number removal requires explicit prefix **or** physical-page match **or** adjacent-page sequence **and** margin-band positioning; years/prices/section numbers/legal refs/Roman letters never match. Auth/protocol phrases + hex marks + OCR barcode-shaped runs are pattern-only (no CV, no PDF reopen). Verticality never triggers or blocks removal by itself. A whole `ReflowUnit` is removed only when every one of its segments qualifies (mixed units kept whole). A repeated header/footer whose text also appears in retained body content is kept with `kept_due_to_ambiguity=True`, never silently dropped. Deterministic entry ids/ordering. Makes the 3 frozen `test_artifacts.py` scenarios GREEN unmodified + `test_artifacts_contract.py` (17 tests: false-positive protection, table-ownership isolation, mixed-unit, ambiguity, determinism).
- [x] T072 [US1] Create `src/solari_converter/reports/removal_log.py` — `RemovalLog` + `RemovalEntry` pydantic + Markdown renderer (FR-011); SC-004 (depends on T028) — **DONE 2026-09-09**: fields match `removal-log.schema.json` verbatim (`element_type` / `kept_due_to_ambiguity`, no new serialized fields); read-only **non-serialized** `entry_type` / `ambiguous` convenience properties; deterministic `removal_entry_id()` (`rm-` + sha1 of page + element_type + sorted lineage — no uuid/timestamp/random); deterministic `render_markdown()`. Makes `test_removal_log_model_exists` / `test_a_built_log_conforms_to_the_committed_schema` GREEN + `test_removal_log.py` (6 tests). The `test_schema_is_in_sync_with_its_model[removal-log]` drift guard stays RED — T128's job (schema regeneration), not this task's.
- [x] T073 [US1] Create `src/solari_converter/model/semantic.py` + `src/solari_converter/transform/build_semantic.py` — `SemanticDocument` (Block union) with per-block `provenance: [segment_id]` + `hint_decisions`; orchestrate reflow → structure → lists → tables → artifacts on the CED in accepted reading order; MUST NOT re-order source segments (FR-064) (depends on T067–T072) — **DONE 2026-09-09**: frozen post-T070/S2 orchestration — `build_tables(ced)` (table ownership from CED alone) → `reflow(ced, excluded_segment_ids=tables.consumed_segment_ids)` → `remove_artifacts` (T071) → `infer_structure`/`reconstruct_lists` (T068/T069) over the retained units → merge. `Block` = `HeadingBlock | ParagraphBlock | ListBlock | ClauseBlock | TableBlock`; **table blocks reuse T070's `TableBlock` verbatim** (`blk-` id unchanged, no remapping); non-table `block_id` = `blk-` + sha1(kind + ordered source segment ids) — no uuid/time/random. Heading classification takes precedence over list/clause when both would apply; consecutive same-family list items group into one `ListBlock`; every block's `hint_decisions` carries both T068's and T069's per-unit decisions. Final block order = ascending minimum accepted-reading-order index per block's own provenance — **never** by `table_id` or `kind`; `StructuralReorder` is the only permitted exception. Proves the 3-way accounting invariant `retained ⊎ collapsed ⊎ removed == set(accepted_reading_order)` (pairwise disjoint) purely from existing T070/T071 data — no additive T070 field needed. Implements the one remaining FR-021 gap: when a stitched table's `StitchRecord.transparent_segment_ids` **survive** T071 removal as retained non-table content, emits an explicit `StructuralReorder(permitted_by="FR-021")` (derived entirely from existing `stitch_records` + accepted-index containment — no new T070 field); `from_order`/`to_order` carry the identical affected-id set; never emitted when unnecessary. `tests/unit/test_build_semantic.py` (12 tests): table-before-reflow ownership, no duplicate/no-silent-loss accounting, collapsed-header + wrapped-cell multi-id accounting, source-order (not kind/table-id) merge, T070 table-identity reuse, FR-021 emitted/not-emitted with identical from/to id sets, no-LLM/network import guard.
- [x] T074 [US1] Architecture test `tests/unit/test_transform_no_llm.py` — no module under `transform/`, `render/`, or `pipeline/export` imports `validate/llm_client` or any LLM symbol (import-graph assertion) — **DONE 2026-09-09 (S1)**: static `ast` import-graph scan of every non-`__init__` module under `transform/` + `render/` (and `pipeline/export.py` when it exists) for `validate.llm_client` / `reconcile.llm_select` and any `llm|openai|anthropic|litellm|ollama` import name; a companion guard asserts `reflow` / `structure` / `lists` were actually in the scanned set. GREEN.
...
- [x] T078 [US1] Create `src/solari_converter/validate/deterministic.py` (shared; the `extract` self-check uses the non-LLM subset) — all the checks in T077 → `ValidationIssue[]` `check_origin="deterministic"` — makes T077 pass (depends on T020, T022, T073; tests T077) — **DONE 2026-09-10 (4E)**: `run_deterministic_checks(*, markdown, semantic, ced, candidates=(), source_page_text=None, gross_divergence_threshold=0.5, ocr_confidence_threshold=70.0) -> DeterministicResult`. `ValidationIssue` frozen dataclass — the six mandatory `validation-report.schema.json` fields + `check_origin` + optional block; `defect_class` stays `None` (T099 assigns the originating stage). Pure/deterministic: fixed check order, every internal emission sorted, ids `det-NNNN` by emission order — no timestamp/random. Coverage denominator excludes `semantic.removed_segment_ids` tokens (logged stage-3 removals). Gross divergence (`match_rate < threshold`) emits one `gross_divergence` issue and suppresses per-token `missing_content`/`numeric_mismatch`; `table_shape`/`duplicated_header`/`ocr_low_confidence` still run. Reading-order check verbatim-localises accepted segments (≥8 chars, unique) and honours a covering `structural_reorder`. Imports no LLM client/symbol (own static test + T074 scope). Ruff clean.
