...
- **`docling`** —
  ...
  *T046 execution record (2026-09-09)* — pinned dependency: **`docling-slim[format-pdf,models-local,convert-core]==2.124.0`**, not the plain `docling==2.124.0` metapackage. Since `docling-slim` 2.92.0 (2026-04-29), the `docling` PyPI name is a thin metapackage equal to `docling-slim[standard]` — same code, same version, published the same day. `standard` additionally pulls a second, unrelated RapidOCR distribution (`rapidocr`, alongside this project's own pinned `rapidocr-onnxruntime`), a CLI/service-client stack (`typer`, `rich`, `websockets`), and office/email/web backends this project never uses (`python-pptx`, `openpyxl`, `mail-parser`, `beautifulsoup4`, chunking `tree-sitter-*`). `format-pdf` + `models-local` + `convert-core` is the minimal extras set yielding the PDF backend (`docling-parse`) and the local layout/table-structure models path A needs, with **no OCR extra of any kind**. Resolved with `uv lock --exclude-newer 2026-09-02T00:00:00Z` (7 days before the 2026-09-09 run date): 47 newly-resolved packages, every one published ≥7 days before the run date (youngest: `docling-core` 2.93.0, `docling-ibm-models` 4.0.1, `regex` 2026.9.3 — each exactly 7 days old) — **no hand-pinned exception was needed**. Materially important resolved versions: `docling-core==2.93.0`, `docling-parse==7.16.0`, `docling-ibm-models==4.0.1`, `torch==2.13.0`, `torchvision==0.28.0`, `transformers==5.8.1`, `huggingface-hub==1.29.0`, `tokenizers==0.22.2`. Zero existing project pins changed. All native-extension packages in the closure ship macOS arm64 + Python 3.13 wheels (verified). `nvidia-*` / `triton` / `pywin32` / `tzdata` entries appear in `uv.lock` (a cross-platform lock) but carry `sys_platform == 'linux'` / `'win32'` markers and do not install on macOS. Runtime policy (`extract/docling_path.py`): `artifacts_path` is mandatory (never `None`); `HF_HUB_OFFLINE=1` / `HF_HUB_DISABLE_TELEMETRY=1` set before any Docling import; `enable_remote_services=False` / `allow_external_plugins=False` set explicitly; `do_ocr=False` explicitly (path C, T050, remains the sole OCR authority — Docling's own OCR auto-selection, which would otherwise probe `ocrmac`/`rapidocr`/`easyocr` on this platform, is never reached); device is explicit `AcceleratorDevice.CPU` (not `"auto"`), per this ledger's local-first / determinism posture. Pinned model artifacts (setup-time only, `scripts/fetch_docling_models.py`; full record `scripts/docling_models/README.md` + `SHA256SUMS`): layout — `docling-project/docling-layout-heron` @ immutable commit `8f39ad3c0b4c58e9c2d2c84a38465abf757272d8` (Apache-2.0, ~172 MB); table structure — `docling-project/docling-models` @ tag `v2.3.0` / commit `fc0f2d45e2218ea24bce5045f58a389aed16dc23` (CDLA-Permissive-2.0, ~213 MB, `accurate` mode). Both fetched once via `huggingface_hub.hf_hub_download` and SHA256-verified against a pinned digest map that `DoclingExtractionPath._verify_artifacts()` re-checks before constructing any Docling component — a missing or mismatched file raises `DoclingModelUnavailable` (fail-closed, no network fallback), converted by `extract/base.py`'s `run_path()` into a `status="failed"` candidate. Verified end-to-end with a real `DocumentConverter` run (`tests/acceptance/test_docling_path_smoke.py`, `-m acceptance`) on Python 3.13.14 / macOS arm64, fully offline.
